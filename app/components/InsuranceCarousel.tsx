        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex items-center justify-center gap-4 mb-10">
            <button
              onClick={prev}
              disabled={offset === 0}
              className="w-10 h-10 flex items-center justify-center text-white/60 hover:text-white disabled:opacity-30 transition-opacity text-2xl shrink-0"
            ...
            <div className="flex items-center gap-8 lg:gap-12 overflow-hidden">
              {insuranceProviders.slice(offset, offset + visibleCount).map((provider) => (
                <Link
                  key={provider.name}
                  href={provider.href}
                  className="flex-shrink-0 flex items-center justify-center h-14 lg:h-16 px-2 group opacity-80 hover:opacity-100 transition-opacity"
                  aria-label={provider.name}
                >
                  <img
                    src={`https://cdn.brandfetch.io/${provider.domain}/theme/light/h/80/w/200/logo?c=${BRANDFETCH_CLIENT_ID}`}
                    alt={provider.name}
                    className="h-8 lg:h-10 w-auto max-w-[160px] object-contain brightness-0 invert"
                    loading="lazy"
                  />
                </Link>
              ))}
