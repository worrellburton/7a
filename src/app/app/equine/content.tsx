              ...
              {horses.map(horse => {
                ...
                return (
                  <React.Fragment key={horse.id}>
                    ...
                    >
                      <td className="px-4 py-3 text-sm font-bold text-foreground whitespace-nowrap">{horse.name}</td>
                      <td className="px-4 py-3 text-sm text-foreground/60 whitespace-nowrap" style={{ fontFamily: 'var(--font-body)' }}>
                        <EditableCell horseId={horse.id} field="age" value={String(horse.age || '')} />
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap" style={{ fontFamily: 'var(--font-body)' }}>
                        <span className={`text-sm ${bodyScoreColor(horse.body_score)}`}>
                          <EditableCell horseId={horse.id} field="body_score" value={String(horse.body_score || '')} className={bodyScoreColor(horse.body_score)} />
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-foreground/60 whitespace-nowrap" style={{ fontFamily: 'var(--font-body)' }}>
                        <EditableCell horseId={horse.id} field="weight" value={horse.weight} />
                      </td>
                      <td className="px-4 py-3 text-sm text-foreground/60 max-w-[160px] truncate" style={{ fontFamily: 'var(--font-body)' }}>
                        <EditableCell horseId={horse.id} field="works_in" value={horse.works_in} />
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${rideableColor(horse.rideable)}`}>
                          {horse.rideable === 'Yes' ? 'Yes' : horse.rideable === 'No' ? 'No' : horse.rideable}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-foreground/60 whitespace-nowrap" style={{ fontFamily: 'var(--font-body)' }}>
                        <EditableCell horseId={horse.id} field="shoe_schedule" value={horse.shoe_schedule} />
                      </td>
