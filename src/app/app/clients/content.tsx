  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, MRN, clinician, payer…" className="flex-1 max-w-sm px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-primary focus:outline-none" />
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-warm-bg/30">
                {([
                  ...
                  { key: 'primary_clinician', label: 'Primary Clinician' },
                ] as const).map((col) => (
                  <th
                    key={col.key}
                    onClick={() => {
                ...
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-foreground/50 uppercase tracking-wider" style={{ fontFamily: 'var(--font-body)' }}>Payer</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-foreground/50 uppercase tracking-wider" style={{ fontFamily: 'var(--font-body)' }}>LOS</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-b border-gray-100 last:border-0 hover:bg-warm-bg/40 transition-colors cursor-pointer" onClick={() => router.push(`/app/clients/${c.id}`)}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-foreground/40">{c.pronouns || ''}{c.age ? ` · Age ${c.age}` : ''}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-xs text-foreground/60 font-mono">{c.mrn || '—'}</td>
                  <td className="px-5 py-3">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_COLORS[c.status] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>
                  ...
                  <td className="px-5 py-3 text-xs text-foreground/70">{c.primary_clinician || '—'}</td>
                  <td className="px-5 py-3 text-xs text-foreground/60">{c.insurance_payer || '—'}</td>
                  <td className="px-5 py-3 text-xs text-foreground/60">{c.status === 'admitted' ? daysIn(c.admission_date) : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
