...
export default function CallAiHover({ call }: { call: Call }) {
  ...
  async function loadScore(force = false) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult(data?.result as ScoreRow);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); setOpen((o) => !o); if (!fetched.current) { fetched.current = true; loadScore(false); } }}
        className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-primary/30 bg-primary/5 text-primary text-[10px] font-semibold hover:bg-primary/10 transition-colors"
      ...
      >
        {loading && !result ? (
          ...
          <span className={`w-5 h-5 -my-0.5 rounded-full text-[9px] font-bold flex items-center justify-center ${scoreColor(result.score)}`}>
            {result.score}
          </span>
        ) : (
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-2 z-50 w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden text-left"
          onMouseEnter={handleEnter}
          onMouseLeave={handleLeave}
        >
          ...
          <div className="p-4 space-y-3 max-h-[26rem] overflow-y-auto text-sm">
            {loading && !result && (
              <p className="text-xs text-foreground/50">Reading the call metadata…</p>
            )}
            {error && (
