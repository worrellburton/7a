      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px]">
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b border-gray-100 last:border-0 hover:bg-warm-bg/40 transition-colors cursor-pointer" onClick={() => router.push(`/feather/clients/${c.id}`)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((c) => (
            <button
              key={c.id}
              onClick={() => router.push(`/feather/clients/${c.id}`)}
