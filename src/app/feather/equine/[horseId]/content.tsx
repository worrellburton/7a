...
export default function HorseContent() {
  ...
  const addVetVisit = async () => {
    ...
    if (user) {
      logActivity({ userId: user.id, type: 'equine.vet_visit_added', targetKind: 'equine', targetId: horse.id, targetLabel: `${horse.name} · ${vetFormReason}`, targetPath: `/feather/equine/${horse.id}`, metadata: { reason: vetFormReason, date: vetFormDate, has_pdf: doc_urls.length > 0 } });
    }
  ...
  const addWeightLog = async () => {
    ...
    if (user) {
      logActivity({ userId: user.id, type: 'equine.weight_logged', targetKind: 'equine', targetId: horse.id, targetLabel: `${horse.name} · ${lbs} lbs`, targetPath: `/feather/equine/${horse.id}`, metadata: { lbs, logged_at: loggedAt } });
    }
    setNewWeightValue('');
  ...
  const deleteWeightLog = async (id: string) => {
    ...
    if (user && horse) {
      logActivity({ userId: user.id, type: 'equine.weight_log_removed', targetKind: 'equine', targetId: horse.id, targetLabel: horse.name, targetPath: `/feather/equine/${horse.id}` });
    }
  };
  ...
  const addFeedLog = async () => {
    ...
    if (user) {
      ...
      logActivity({ userId: user.id, type: 'equine.feed_logged', targetKind: 'equine', targetId: horse.id, targetLabel: label, targetPath: `/feather/equine/${horse.id}`, metadata: { feed_type: newFeedType.trim(), amount: amt, unit: newFeedUnit } });
    }
    setNewFeedType('');
    setNewFeedAmount('');
    setNewFeedNotes('');
  ...
  const deleteFeedLog = async (id: string) => {
    ...
    if (user && horse) {
      logActivity({ userId: user.id, type: 'equine.feed_log_removed', targetKind: 'equine', targetId: horse.id, targetLabel: horse.name, targetPath: `/feather/equine/${horse.id}` });
    }
  };
  ...
  const removeVetVisit = async (visitIndex: number) => {
    ...
    setHorse({ ...horse, vet_visits: newVisits });
    if (user) {
      logActivity({ userId: user.id, type: 'equine.vet_visit_removed', targetKind: 'equine', targetId: horse.id, targetLabel: `${horse.name}${removed?.reason ? ' · ' + removed.reason : ''}`, targetPath: `/feather/equine/${horse.id}` });
    }
  };
  ...
  if (notFound || !horse) {
    return (
      <div className="p-4 sm:p-6 lg:p-10">
        <Link href="/feather/equine" className="text-sm text-primary hover:underline" style={{ fontFamily: 'var(--font-body)' }}>
          &larr; Back to Horses
        </Link>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-5xl">
      ...
      <button
        onClick={() => router.push('/feather/equine')}
