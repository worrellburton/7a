          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-warm-bg/40 text-[11px] uppercase tracking-wider text-foreground/55">
                <tr>
                  <Th className="w-72">Source</Th>
                  <Th>Anchor → Target</Th>
                  <Th>Type</Th>
                  <Th className="text-right">Page score</Th>
                  <Th className="text-right">Last seen</Th>
                  <Th className="text-right w-12">Notes</Th>
                  <Th className="text-right w-10"><span className="sr-only">Delete</span></Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-black/5">
                {visibleRows.map((r, i) => (
                  <tr key={`${r.source_url}-${i}`} className="align-top">
                    ...
                    {/* Delete affordance — visible only for manual
                        backlink rows (those that have an entry in
                        manualByKey). Semrush-synced rows can't be
                        deleted from the UI; they live in the
                        snapshot blob and roll over with each sync. */}
                    <Td className="text-right">
                      {manualByKey[r.source_url] ? (
                        <button
                          type="button"
                          onClick={async () => {
                            if (!confirm('Remove this manual backlink?')) return;
                            const id = manualByKey[r.source_url].id;
                            const res = await fetch(`/api/seo/backlinks/manual?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
                            if (!res.ok) {
                              const json = await res.json().catch(() => ({}));
                              setError(json?.error ?? `HTTP ${res.status}`);
                            }
                          }}
                          title="Remove manual backlink"
                          aria-label="Remove manual backlink"
      ...
      {snapshot?.target ? (
        <p className="text-[11px] text-foreground/40 mt-3">
          ...
          {snapshot?.total_in_snapshot != null
            ...
            : ''}
          {' · '}
          <Link href="/feather/seo/refdomains" className="underline decoration-dotted hover:text-foreground">
