...
export default function FacilitiesContent() {
  ...
  // Auto-open the new-issue form when arriving via /feather/facilities?new=1
  ...
  const updateStatus = async (id: string, status: Status) => {
    ...
    if (user?.id) {
      ...
      logActivity({ userId: user.id, type: 'facilities.status_changed', targetKind: 'facilities_issue', targetId: id, targetLabel: `${it?.location || ''} — ${it?.issue || ''} → ${status}`, targetPath: '/feather/facilities', metadata: { status } });
    }
  };
  ...
  const updatePriority = async (id: string, priority: Priority) => {
    ...
    if (user?.id) {
      const it = items.find((i) => i.id === id);
      logActivity({ userId: user.id, type: 'facilities.priority_changed', targetKind: 'facilities_issue', targetId: id, targetLabel: `${it?.location || ''} — ${it?.issue || ''} → ${priority}`, targetPath: '/feather/facilities', metadata: { priority } });
    }
  };
  ...
  const saveIssueText = async (id: string) => {
    ...
    if (user?.id) {
      const it = items.find((i) => i.id === id);
      logActivity({ userId: user.id, type: 'facilities.issue_updated', targetKind: 'facilities_issue', targetId: id, targetLabel: `${it?.location || ''} — ${trimmed}`, targetPath: '/feather/facilities' });
    }
  };
  ...
  const deleteIssue = async (id: string) => {
    ...
    setExpandedId(null);
    if (user?.id) {
      logActivity({ userId: user.id, type: 'facilities.deleted', targetKind: 'facilities_issue', targetId: id, targetLabel: `${removed?.location || ''} — ${removed?.issue || ''}`, targetPath: '/feather/facilities' });
    }
  };
      ...
      {viewingPhoto && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setViewingPhoto(null)}>
          ...
          <button
            ...
            className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-3 h-10 rounded-full bg-red-600 text-white text-xs font-semibold hover:bg-red-700 transition-colors shadow-md"
            aria-label="Delete photo"
          >
