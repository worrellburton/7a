...
export default function BillingContent() {
  ...
  const createClaim = async () => {
    ...
    if (dbAvailable) {
      ...
      if (result?.id) {
        ...
        if (user?.id) logActivity({ userId: user.id, type: 'billing.claim_created', targetKind: 'billing_claim', targetId: result.id, targetLabel: `$${data.charge_amount} · ${data.procedure_code}`, targetPath: '/feather/billing' });
      }
    } else {
  ...
  const confirmStediSubmit = async () => {
    ...
    try {
      ...
      if (user?.id) logActivity({ userId: user.id, type: 'billing.claim_submitted', targetKind: 'billing_claim', targetId: claim.id, targetLabel: `$${claim.charge_amount} · ${newStatus}`, targetPath: '/feather/billing', metadata: { status: newStatus, stedi_claim_id: stediClaimId } });
    } catch (err) {
  ...
  const moveClaimToStage = async (claimId: string, newStatus: string) => {
    ...
    if (user?.id) logActivity({ userId: user.id, type: 'billing.claim_status_changed', targetKind: 'billing_claim', targetId: claimId, targetLabel: `$${claim?.charge_amount ?? ''} · ${newStatus}`, targetPath: '/feather/billing', metadata: { status: newStatus } });
  };
