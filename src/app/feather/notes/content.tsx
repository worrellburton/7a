...
export default function NotesContent() {
  ...
  async function saveNote(status: NoteStatus) {
    ...
    if (editingNote) {
      ...
      if (res && (res as { ok?: boolean }).ok) {
        ...
        logActivity({ userId: user.id, type: 'note.updated', targetKind: 'note', targetId: editingNote.id, targetLabel: `${TYPE_LABELS[noteType]} · ${selectedClient.name}`, targetPath: '/feather/notes' });
      }
    } else {
      const data = await db({
        action: 'insert',
      ...
      if (data && (data as NoteRow).id) {
        ...
        logActivity({ userId: user.id, type: 'note.created', targetKind: 'note', targetId: (data as NoteRow).id, targetLabel: `${TYPE_LABELS[noteType]} · ${selectedClient.name}`, targetPath: '/feather/notes' });
      }
    }
    setSaving(false);
    setView('list');
  ...
  async function deleteNote(note: NoteRow) {
    ...
    if (res && (res as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'note.deleted', targetKind: 'note', targetId: note.id, targetLabel: `${TYPE_LABELS[note.note_type]} · ${client?.name || 'Unknown'}`, targetPath: '/feather/notes' });
    }
  }
