...
export default function AskPolicies() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col gap-1.5 px-4 sm:px-6">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[11px] font-semibold text-foreground/60 uppercase tracking-wider" style={{ fontFamily: 'var(--font-body)' }}>
          Ask about policies
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {history.length === 0 ? (
          <div className="px-2.5 py-2">
            <p className="text-[10px] text-foreground/40 mb-1.5" style={{ fontFamily: 'var(--font-body)' }}>Try:</p>
            <div className="flex flex-wrap gap-1">
              {SAMPLE_QUESTIONS.slice(0, 4).map((s) => (
                <button
                  key={s}
                  onClick={() => ask(s)}
                  className="px-2 py-0.5 text-[10.5px] text-foreground/70 bg-warm-bg hover:bg-primary/10 hover:text-primary rounded-full transition-colors"
                  style={{ fontFamily: 'var(--font-body)' }}
                >
                  {s}
                </button>
