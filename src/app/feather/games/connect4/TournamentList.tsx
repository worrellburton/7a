...
export default function TournamentList() {
  ...
  const onCreate = useCallback(async () => {
    ...
    setName('');
    router.push(`/feather/games/connect4?tournament=${created.id}`);
    ...
    <section className="rounded-2xl border border-black/10 bg-white/60 px-4 py-3">
      ...
      {active.length > 0 && (
        <ul className="space-y-1 mb-2">
          {active.map((t) => (
            <li key={t.id}>
              <Link href={`/feather/games/connect4?tournament=${t.id}`} className="flex items-center gap-2 px-3 py-2 rounded-lg text-[12.5px] hover:bg-warm-bg/60" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      {past.length > 0 && (
        <>
          ...
          <ul className="space-y-1">
            {past.map((t) => (
              <li key={t.id}>
                <Link href={`/feather/games/connect4?tournament=${t.id}`} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[12px] text-foreground/55 hover:bg-warm-bg/60" style={{ fontFamily: 'var(--font-body)' }}>
                  <span className="truncate flex-1">{t.name}</span>
