...
export default function BracketLobby() {
  ...
  const challenge = useCallback(
    async (opponentId: string) => {
      ...
      router.push(`/feather/games/connect4?match=${newMatch.id}`);
    },
  ...
  const onCellClick = useCallback(
    (a: UserLite | null, b: UserLite | null) => {
      ...
      if (existing && (existing.status !== 'complete' && existing.status !== 'forfeit')) {
        router.push(`/feather/games/connect4?match=${existing.id}`);
        return;
      }
      if (existing) {
        router.push(`/feather/games/connect4?match=${existing.id}`);
...
function YourMatchSpotlight({
  ...
  if (liveOrOpen) {
    ...
    return (
      <Link
        href={`/feather/games/connect4?match=${liveOrOpen.id}`}
        ...
        {matches.map((m) => {
          ...
          return (
            <li key={m.id} className="shrink-0">
              <Link
                href={`/feather/games/connect4?match=${m.id}`}
...
function MatchPaneItem({
  ...
  return (
    <li>
      <Link
        href={`/feather/games/connect4?match=${match.id}`}
