...
// User Permissions page (renamed from /feather/super-admin in Phase 1).
// Same column layout as /feather/team — User | Viewing | Department |
...
// reassign without bouncing out to /feather/team. Permissions overrides
...
function pageLabelFromPath(path: string | null): string {
  if (!path) return '';
  if (path === '/feather' || path === '/feather/') return 'Home';
  if (path.startsWith('/feather/job-descriptions')) return 'Job Descriptions';
  if (path.startsWith('/feather/sign')) return 'Signing';
...
export default function UserPermissionsContent() {
  ...
  // Custom-title state for the Job Title cell — same UX as /feather/team:
  ...
  async function toggleAdmin(u: AppUser, next: boolean) {
    ...
    if (!res || (typeof res === 'object' && 'error' in res)) {
      ...
      logActivity({
        ...
        targetPath: '/feather/user-permissions',
                ...
                {sortedUsers.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="border-b border-gray-100 last:border-b-0 hover:bg-warm-bg/30 transition-colors">
                      ...
                      {/* Job Title — same dropdown + inline custom-title input as /feather/team */}
