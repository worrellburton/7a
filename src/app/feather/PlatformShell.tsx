...
import { VisiblePagesContext } from './VisiblePages';

// Department glyphs for the two-level nav's root panel, keyed by name
// keywords so admin renames keep matching. Unknown departments fall
// back to their initial letter.
function departmentGlyph(name: string): React.ReactNode | null {
  const n = name.toLowerCase();
  const wrap = (children: React.ReactNode) => (
    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" aria-hidden="true">{children}</svg>
  );
  if (n.includes('admin')) return wrap(<><rect x="3" y="7" width="18" height="13" rx="2" /><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /><path d="M3 13h18" /></>);
  if (n.includes('alumni')) return wrap(<><path d="M22 9L12 4 2 9l10 5 10-5z" /><path d="M6 11v5c0 1.5 2.7 3 6 3s6-1.5 6-3v-5" /></>);
  if (n.includes('culture') || n.includes('fun')) return wrap(<><path d="M12 3l1.9 5.8L20 10l-5 3.6L16.5 20 12 16.5 7.5 20 9 13.6 4 10l6.1-1.2z" /></>);
  if (n.includes('equine') || n.includes('horse')) return wrap(<><path d="M4 20c0-6 3-11 9-13l6-3-1 5c2 1 3 3 3 5" /><path d="M13 7c-4 2-6 6-6 13" /><path d="M17 9l2 2" /></>);
  if (n.includes('kitchen')) return wrap(<><path d="M4 3v7a2 2 0 0 0 2 2v9" /><path d="M8 3v7" /><path d="M6 3v7" /><path d="M15 3c-1.5 2-2 4-2 6 0 2 1 3 2 3v9" /><path d="M15 3c1.5 2 2 4 2 6 0 2-1 3-2 3" /></>);
  if (n.includes('clinical') || n.includes('medical')) return wrap(<><path d="M19 14c1.5-1.5 2-3.4 2-5a5 5 0 0 0-9-3 5 5 0 0 0-9 3c0 1.6.5 3.5 2 5l7 7 7-7z" /><path d="M8 12h2l1.5-3 2 5L15 12h2" /></>);
  if (n.includes('marketing') || n.includes('admissions')) return wrap(<><path d="M3 11l14-5v12L3 13v-2z" /><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6" /><path d="M17 6l4-2v16l-4-2" /></>);
  if (n === 'more') return wrap(<><circle cx="5" cy="12" r="1.6" /><circle cx="12" cy="12" r="1.6" /><circle cx="19" cy="12" r="1.6" /></>);
  return null;
}
...
const pageIcons: Record<string, React.ReactNode> = {
  ...
  // CTM (legacy CallTrackingMetrics) — a trend-line icon so the
  // attribution/analytics surface reads distinctly from the live
  // Aircall phone icon above.
  '/feather/ctm': (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 3v18h18" />
...
export default function PlatformShell({ children }: { children: React.ReactNode }) {
  ...
  // Everything the viewer can reach (nav + popup surfaces) — provided
  // via VisiblePagesContext so in-shell surfaces (Home-page search)
  // reuse the finished permission filtering instead of re-deriving it.
  const allVisiblePages = [...visibleNavPages, ...popupPages.filter(canSeePage)];
  ...
  const recencyOrderedPages = useMemo(() => {
    ...
    // My Profile is pinned to position 2 (right under Home) for
    // EVERYONE now — staff and alumni alike. It may live in either
    // the nav or the popup section, so check both.
    const profilePage =
      visibleNavPages.find((p) => p.path === PROFILE_PATH)
      ?? popupPages.find((p) => p.path === PROFILE_PATH)
      ?? null;
    const pinned = new Set<string>([HOME_PATH, PROFILE_PATH]);
  ...
  const navPinnedPaths = useMemo(() => {
    const HOME_PATH = isAlumni ? '/feather/alumni' : '/feather';
    const PROFILE_PATH = isAlumni ? '/feather/alumni/profile' : '/feather/profile';
    return new Set([HOME_PATH, PROFILE_PATH]);
  }, [isAlumni]);
  ...
  return (
    <VisiblePagesContext.Provider value={allVisiblePages}>
    <div className="flex min-h-screen app-shell relative">
      ...
      <aside data-sidebar-rail className="group/sidebar w-16 hover:w-64 rail-open:w-64 transition-[width] duration-200 ease-out shrink-0 hidden lg:block relative z-30">
        ...
        <>
        {/* The sidebar "Search pages…" box moved to the top of the
            Home page (with autocomplete) — cmd/ctrl-K still opens the
            global palette from anywhere. navSearch stays wired so the
            search-results branch below is inert, not broken. */}
                    ...
                    <>
                      ...
                      {navPanelData.sections.map((s) => (
                        <button
                          ...
                          style={{ fontFamily: 'var(--font-body)' }}
                        >
                          {/* Collapsed-rail glyph: dept-tinted icon (initial
                              letter as the fallback for unmapped names). */}
                          <span
