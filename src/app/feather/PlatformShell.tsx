...
/**
 * Feather brand mark for the sidebar / mobile drawer header. The app
 * is "Feather" — a copper-gradient pill containing a hand-drawn
 * feather glyph, with a soft pulsing radial halo behind it. Two
 * sizes — md for the pinned desktop sidebar, sm for the compact
...
function SevenArrowsLogo({ size = 'md' }: { size?: 'sm' | 'md' }) {
  ...
  const dim = big ? 26 : 20;
  return (
    <span className="relative inline-flex items-center" aria-label="Feather">
      <style jsx>{`
      ...
      />
      {/* Mark — copper-gradient rounded square with a feather glyph */}
      <span
        className={`relative inline-flex items-center justify-center rounded-xl ${big ? 'w-9 h-9' : 'w-7 h-7'} text-white`}
        style={{
          background:
      ...
      >
        <FeatherGlyph size={dim} className="drop-shadow-sm" />
      </span>
    </span>
  );
}
...
/**
 * Hand-drawn feather glyph used for the Feather brand mark. The shape
 * is built from a curved spine with a softly tapered vane on either
 * side; barbs are short cross-strokes that get longer toward the base
 * so the silhouette reads as a quill rather than a generic leaf.
 */
function FeatherGlyph({ size = 24, className }: { size?: number; className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      width={size}
      ...
      className={className}
      aria-hidden="true"
      style={{ animation: 'sa-logo-feather 4.4s ease-in-out infinite', transformOrigin: '50% 80%' }}
    >
      {/* Vane silhouette */}
      <path
        d="M5.5 18.5c0-7 5-13 13-15-1 7-3 11-6 13.5-2.4 2-5.2 2.4-7 1.5z"
        fill="currentColor"
        fillOpacity="0.18"
      />
      {/* Spine */}
      <path d="M5.5 18.5L18.5 3.5" />
      {/* Barbs — short cross-strokes along the spine */}
      <path d="M9 15l3-3" />
      <path d="M11 13l4-2" />
      <path d="M13 11l4-1.5" />
      <path d="M14.5 9l3.2-1" />
      {/* Quill tail */}
      <path d="M5.5 18.5l-1.5 1.5" />
    </svg>
  );
}
...
export default function PlatformShell({ children }: { children: React.ReactNode }) {
  ...
  // the badge on /feather/website-requests; the structure leaves room for
  // other inboxes later.
  ...
  //   1. Per-user override (set by a super admin via /feather/user-permissions
  ...
  const canSeePage = (item: { path: string; adminOnly: boolean }) => {
    ...
    // admin granted via /feather/user-permissions → Departments tab.
  ...
  // filtered out client-side so the admin toggle in /feather/pages flips
  ...
  // Skipped when the user is already on /feather/finance (pointless) or
    ...
    <div className="flex min-h-screen app-shell relative">
              ...
              return (
                <Link
                  ...
                  onClick={(e) => {
                    ...
                    // string (e.g. /feather/calls?tab=operators), Next.js's
        ...
        <div ref={userMenuRef} className="relative p-3 border-t border-gray-100">
          {userMenuOpen && (
            ...
            >
              ...
              {popupPages.filter(canSeePage).filter((p) => p.path !== '/feather/profile').map((item) => (
                <Link
                  key={item.path}
                  href={item.path}
                  onClick={() => setUserMenuOpen(false)}
      ...
      <div data-platform-main className="flex-1 bg-warm-bg overflow-auto relative">
        ...
        <div className="lg:hidden sticky top-0 z-30 flex items-center justify-between px-4 h-14 bg-white/90 backdrop-blur border-b border-gray-100">
          ...
          <Link href="/feather" aria-label="Seven Arrows Recovery">
          ...
          <div className="lg:hidden fixed inset-0 z-[70]" role="dialog" aria-modal="true">
            ...
            <aside className="absolute inset-y-0 left-0 w-[82%] max-w-[320px] bg-white border-r border-gray-100 shadow-2xl flex flex-col overflow-hidden animate-drawer-slide">
              ...
              <div className="flex items-center justify-between p-5 border-b border-gray-100 shrink-0">
                <Link href="/feather" className="flex items-center gap-2.5">
                  <SevenArrowsLogo />
              ...
              {mobileAccountOpen && (
                <>
                  {popupPages.filter(canSeePage).filter((p) => p.path !== '/feather/profile').length > 0 && (
        ...
        {/* Same-page viewers — shown on every /feather/* page */}
        <PageViewers />
