  ...
  if (error || !department) {
    return (
      <div className={embedded ? 'p-6' : 'p-4 sm:p-6 lg:p-10'}>
        {!embedded && (
          <Link href="/feather/finance" className="text-xs font-semibold text-primary hover:underline">← Back to Finance</Link>
        )}
  ...
  const body = (
    <>
      {!embedded && (
        <>
          {/* Breadcrumb */}
          <Link href="/feather/finance" className="inline-flex items-center gap-1 text-xs font-semibold text-foreground/50 hover:text-primary transition-colors mb-3" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      {!realmId && !embedded && (
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 mb-5">
          <p className="text-xs text-amber-800" style={{ fontFamily: 'var(--font-body)' }}>
            No QuickBooks company selected. Return to <Link href="/feather/finance" className="font-semibold underline">Finance</Link> and pick a realm to see live account data.
          </p>
        </div>
      )}
