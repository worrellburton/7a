...
export default function MessagesPanel({ token }: { token: string | null }) {
  ...
  const launcher = (
    <button
      ...
      className="group inline-flex items-center gap-2.5 rounded-2xl border border-white/70 bg-white/55 supports-[backdrop-filter]:bg-white/35 backdrop-blur-xl px-4 py-2.5 shadow-[0_10px_30px_-18px_rgba(60,48,42,0.35)] hover:bg-white/70 transition-colors"
      style={{ fontFamily: 'var(--font-body)' }}
    >
      <span className="relative inline-flex items-center justify-center w-8 h-8 rounded-xl bg-primary/10 text-primary">
      ...
      <span className="flex flex-col items-start leading-tight">
        ...
        <span className="text-[11px] text-foreground/50">
          {threads.length > 0 ? `${threads.length} conversation${threads.length === 1 ? '' : 's'}` : 'Open SMS inbox'}
        </span>
      </span>
    </button>
  );
