...
export default function GroupsContent() {
  ...
  async function saveGroup() {
    ...
    if (editingGroup) {
      ...
      if (res && (res as { ok?: boolean }).ok) {
        ...
        if (user) logActivity({ userId: user.id, type: 'group.updated', targetKind: 'group', targetId: editingGroup.id, targetLabel: payload.name, targetPath: '/feather/groups' });
        closeModal();
      }
    } else {
      ...
      if (data && (data as Group).id) {
        ...
        if (user) logActivity({ userId: user.id, type: 'group.created', targetKind: 'group', targetId: (data as Group).id, targetLabel: payload.name, targetPath: '/feather/groups' });
        closeModal();
      }
    }
    setSaving(false);
  ...
  async function deleteGroup(id: string, groupName: string) {
    ...
    if (result && (result as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'group.deleted', targetKind: 'group', targetId: id, targetLabel: groupName, targetPath: '/feather/groups' });
    }
  }
