...
interface SiteImage {
  ...
  // AI provenance — populated when an AI image gets promoted into
  // the library via the /api/content/[id] save-selection hook
  // (see migration 20260521_site_images_ai_provenance.sql).
  is_ai?: boolean | null;
...
export default function ImagesContent() {
  ...
  // Real vs AI tab — defaults to All. AI images land here via the
  // /api/content/[id] save-selection hook (any blog_image whose
  // provider is not 'library' gets promoted into site_images with
  // is_ai=true).
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10">
      ...
      {loading ? (
        ...
        <>
          {/* Real vs AI tab strip. All = the full library; Real =
              uploads + kaizen-processed; AI = the subset promoted
              from the /app/content AI pipeline (is_ai=true). Counts
              update live as the tab changes since they're derived
              from the same images array. */}
          {(() => {
            const aiCount = images.filter((i) => i.is_ai === true).length;
            const realCount = images.length - aiCount;
            const tone = (active: boolean) => active
              ? 'bg-foreground text-white border-foreground'
              : 'bg-white text-foreground/65 border-black/10 hover:bg-warm-bg/60';
            return (
              <div className="mb-4 flex items-center gap-1.5 flex-wrap" style={{ fontFamily: 'var(--font-body)' }}>
                <button type="button" onClick={() => setTab('all')} className={`px-3 py-1.5 rounded-md text-[11.5px] font-semibold border ${tone(tab === 'all')}`}>
                  All <span className="ml-1 text-[10.5px] opacity-70 tabular-nums">{images.length}</span>
                </button>
                <button type="button" onClick={() => setTab('real')} className={`px-3 py-1.5 rounded-md text-[11.5px] font-semibold border ${tone(tab === 'real')}`}>
                  Real <span className="ml-1 text-[10.5px] opacity-70 tabular-nums">{realCount}</span>
                </button>
                <button type="button" onClick={() => setTab('ai')} className={`px-3 py-1.5 rounded-md text-[11.5px] font-semibold border ${tone(tab === 'ai')}`}>
                  AI <span className="ml-1 text-[10.5px] opacity-70 tabular-nums">{aiCount}</span>
                </button>
                {tab === 'ai' && aiCount === 0 && (
                  <span className="ml-2 text-[11.5px] text-foreground/50 italic">
                    No AI images yet — they land here when an editor saves an AI-generated image on a blog.
                  </span>
                )}
              </div>
            );
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {images
            .filter((i) => tab === 'all' ? true : tab === 'ai' ? i.is_ai === true : i.is_ai !== true)
            .map((img) => (
            <div key={img.id} className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg transition-shadow">
              ...
              >
                ...
                />
                {/* AI badge — small chip in the top-left corner so
                    a user scrolling the 'All' tab can tell at a
                    glance which tiles came from the blog AI pipeline
                    vs an upload. Hidden on the 'AI' tab (every tile
                    there is AI by definition) to reduce noise. */}
                {img.is_ai && tab !== 'ai' && (
                  <span
                    className="absolute top-2 left-2 z-10 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-violet-100/95 text-violet-800 border border-violet-200 backdrop-blur-sm"
                    title={img.ai_provider ? `Generated by ${img.ai_provider}` : 'AI-generated'}
                    style={{ fontFamily: 'var(--font-body)' }}
                  >
                    AI
                  </span>
