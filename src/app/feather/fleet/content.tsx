...
export default function FleetContent() {
  ...
  const addVehicle = async () => {
    ...
    if (result && result.id) {
      ...
      if (user) logActivity({ userId: user.id, type: 'fleet.vehicle_added', targetKind: 'fleet_vehicle', targetId: result.id, targetLabel: newName.trim(), targetPath: '/feather/fleet' });
      setNewName('');
      setNewType('');
  ...
  const deleteVehicle = async (v: Vehicle) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'fleet.vehicle_removed', targetKind: 'fleet_vehicle', targetId: v.id, targetLabel: v.vehicle_name, targetPath: '/feather/fleet' });
    ...
    if (url) {
      ...
      if (result && result.id) {
        ...
        if (user) {
          ...
          logActivity({ userId: user.id, type: target.replaceDocId ? 'fleet.doc_replaced' : 'fleet.doc_uploaded', targetKind: 'fleet_vehicle', targetId: target.vehicleId, targetLabel: `${v?.vehicle_name || 'Vehicle'} · ${target.docType}`, targetPath: '/feather/fleet', metadata: { doc_type: target.docType, file_name: file.name } });
        }
      }
    }
    setUploading(null);
  ...
  const deleteDoc = async (doc: VehicleDocument) => {
    ...
    if (user) {
      ...
      logActivity({ userId: user.id, type: 'fleet.doc_removed', targetKind: 'fleet_vehicle', targetId: doc.vehicle_id, targetLabel: `${v?.vehicle_name || 'Vehicle'} · ${doc.file_name}`, targetPath: '/feather/fleet' });
    }
  };
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'fleet.vehicle_status_changed', targetKind: 'fleet_vehicle', targetId: id, targetLabel: `${v?.vehicle_name || 'Vehicle'} → ${status}`, targetPath: '/feather/fleet', metadata: { status } });
  };
