...
export default function RCMPipelineContent() {
  ...
  const moveClaimToStage = useCallback(async (claimId: string, newStatus: string) => {
    ...
    if (user?.id) logActivity({ userId: user.id, type: 'billing.claim_status_changed', targetKind: 'billing_claim', targetId: claimId, targetLabel: `$${claim?.charge_amount ?? ''} · ${newStatus}`, targetPath: '/feather/rcm-pipeline', metadata: { status: newStatus } });
  }, [claims, user]);
