...
export function TourDetailPanel({
  ...
  const addGuest = useCallback(async () => {
    ...
    if (inserted && inserted.id) {
      ...
      if (user) logActivity({ userId: user.id, type: 'tour.guest_added', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${newGuest.name.trim()}`, targetPath: `/feather/tours` });
    }
  ...
  const updateGuest = useCallback(async (id: string, patch: Partial<TourGuest>) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.guest_updated', targetKind: 'tour', targetId: tour.id, targetLabel: tour.tour_name, targetPath: `/feather/tours` });
  ...
  const deleteGuest = useCallback(async (id: string) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.guest_removed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${removed?.name ? ' · ' + removed.name : ''}`, targetPath: `/feather/tours` });
  ...
  const addPresentation = useCallback(async () => {
    ...
    if (inserted && inserted.id) {
      ...
      if (user) logActivity({ userId: user.id, type: 'tour.presentation_added', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${newPres.title.trim()}`, targetPath: `/feather/tours` });
    }
  ...
  const deletePresentation = useCallback(async (id: string) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.presentation_removed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${removed?.title ? ' · ' + removed.title : ''}`, targetPath: `/feather/tours` });
  ...
  const togglePresentationSignup = useCallback(async (presentationId: string) => {
    ...
    if (mine) {
      ...
      logActivity({ userId: user.id, type: 'tour.presentation_signup_removed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${pres?.title ? ' · ' + pres.title : ''}`, targetPath: `/feather/tours` });
    } else {
      ...
      if (inserted && inserted.id) {
        ...
        logActivity({ userId: user.id, type: 'tour.presentation_signup_added', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${pres?.title ? ' · ' + pres.title : ''}`, targetPath: `/feather/tours` });
      }
    }
  ...
  const addBdStaff = useCallback(async () => {
    ...
    if (inserted && inserted.id) {
      ...
      setNewBdUserId('');
      setNewBdRole('');
      if (user) logActivity({ userId: user.id, type: 'tour.staff_added', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${u.full_name || 'Staff'}`, targetPath: `/feather/tours` });
    }
  ...
  const removeBdStaff = useCallback(async (id: string) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.staff_removed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${removed?.staff_name ? ' · ' + removed.staff_name : ''}`, targetPath: `/feather/tours` });
  ...
  const assignSchedule = useCallback(async (kind: ScheduleKind, userId: string) => {
    ...
    if (user) {
      ...
      logActivity({ userId: user.id, type: 'tour.schedule_assigned', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${kindLabel} → ${u?.full_name || 'Unassigned'}`, targetPath: `/feather/tours` });
    }
  ...
  const setScheduleStatus = useCallback(async (kind: ScheduleKind, status: ScheduleItem['approval_status']) => {
    ...
    if (user) {
      const kindLabel = SCHEDULE_KINDS.find((k) => k.kind === kind)?.label || kind;
      logActivity({ userId: user.id, type: 'tour.schedule_status_changed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${kindLabel} → ${status}`, targetPath: `/feather/tours`, metadata: { status } });
    }
  ...
  const addRoom = useCallback(async () => {
    ...
    if (inserted && inserted.id) {
      ...
      setNewRoomName('');
      if (user) logActivity({ userId: user.id, type: 'tour.room_added', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${(inserted as TourRoom).room_name}`, targetPath: `/feather/tours` });
    }
  ...
  const updateRoom = useCallback(async (id: string, patch: Partial<TourRoom>) => {
    ...
    if (user && prev) {
      ...
      logActivity({ userId: user.id, type: 'tour.room_updated', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name} · ${prev.room_name}${summary}`, targetPath: `/feather/tours` });
    }
  ...
  const deleteRoom = useCallback(async (id: string) => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.room_removed', targetKind: 'tour', targetId: tour.id, targetLabel: `${tour.tour_name}${removed?.room_name ? ' · ' + removed.room_name : ''}`, targetPath: `/feather/tours` });
  ...
  const handleDeleteTour = useCallback(async () => {
    ...
    if (user) logActivity({ userId: user.id, type: 'tour.deleted', targetKind: 'tour', targetId: tour.id, targetLabel: tour.tour_name, targetPath: `/feather/tours` });
    onDelete();
