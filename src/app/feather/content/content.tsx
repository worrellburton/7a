...
export default function ContentLanding() {
  ...
  const hasContentAccess = isSuperAdmin || isAdmin || inMarketing || userOverrides['/feather/content'] === true;
  ...
  const createBlog = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/feather/content/${id}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setCreating(false);
    }
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ fontFamily: 'var(--font-body)' }}>
      <header className="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/feather/content/analytics"
                  ...
                  return (
                    <li key={`static-${ep.slug}`}>
                      <BlogRow
                        ...
                        href={`/feather/content/static/${ep.slug}`}
                        externalHref={path}
                        hidden={hidden}
...
function RoadmapTab() {
  ...
  const build = useCallback(async (row: RoadmapRow) => {
    ...
    if (row.blog_id) {
      router.push(`/feather/content/${row.blog_id}`);
      return;
    }
    ...
    try {
      ...
      router.push(`/feather/content/${json.id}`);
    } finally {
      setBuildingId(null);
    }
