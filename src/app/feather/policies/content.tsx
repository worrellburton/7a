...
export default function PoliciesContent() {
  ...
  async function savePolicy() {
    ...
    if (data && (data as Policy).id) {
      ...
      logActivity({ userId: user.id, type: 'policy.created', targetKind: 'policy', targetId: (data as Policy).id, targetLabel: payload.name, targetPath: '/feather/policies' });
      closeAdd();
    }
    setSaving(false);
  }
  ...
  async function markReviewed(p: Policy) {
    ...
    if (res && (res as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'policy.reviewed', targetKind: 'policy', targetId: p.id, targetLabel: p.name, targetPath: '/feather/policies' });
    }
  }
  ...
  async function markRevised(p: Policy) {
    ...
    if (res && (res as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'policy.revised', targetKind: 'policy', targetId: p.id, targetLabel: p.name, targetPath: '/feather/policies' });
    }
  }
  ...
  async function deletePolicy(p: Policy) {
    ...
    if (res && (res as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'policy.deleted', targetKind: 'policy', targetId: p.id, targetLabel: p.name, targetPath: '/feather/policies' });
    }
  }
  ...
  async function applyBulkSection() {
    ...
    if (user) {
      okIds.forEach((id) => {
        ...
        if (p) logActivity({ userId: user.id, type: 'policy.updated', targetKind: 'policy', targetId: id, targetLabel: p.name, targetPath: '/feather/policies', metadata: { section: bulkSection } });
      });
    }
  ...
  async function applyBulkDepartment() {
    ...
    if (user) {
      ...
      okIds.forEach((id) => {
        const p = policies.find((x) => x.id === id);
        if (p) logActivity({ userId: user.id, type: 'policy.updated', targetKind: 'policy', targetId: id, targetLabel: p.name, targetPath: '/feather/policies', metadata: { department_id: nextDepartmentId, department: deptName } });
      });
    }
    setSelectedIds(new Set());
    setApplyingBulk(false);
  ...
  async function bulkDelete() {
    ...
    if (user) {
      okIds.forEach((id) => {
        const p = policies.find((x) => x.id === id);
        if (p) logActivity({ userId: user.id, type: 'policy.deleted', targetKind: 'policy', targetId: id, targetLabel: p.name, targetPath: '/feather/policies' });
      });
    }
    setSelectedIds(new Set());
    setApplyingBulk(false);
  ...
  async function saveRename(p: Policy) {
    ...
    if (res && (res as { ok?: boolean }).ok) {
      ...
      if (user) logActivity({ userId: user.id, type: 'policy.renamed', targetKind: 'policy', targetId: p.id, targetLabel: next, targetPath: '/feather/policies', metadata: { from: p.name, to: next } });
    }
    setEditingNameId(null);
  }
