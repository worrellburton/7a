...
// sends the visitor to `{SITE_URL}/auth/callback?next=/feather` with a
...
// and then redirect to whatever `next` asked for (defaulting to /feather
// for admin sign-ins).
//
...
//      request to /feather already has the user attached (no flicker of
...
export async function GET(req: Request) {
  ...
  const nextParam = url.searchParams.get('next') || '/feather';
