...
export async function POST(req: NextRequest) {
  ...
  // Credit the adder with one 'Data Entry' log per inserted
  // contact so the outreach activity feed (and the home log-rain)
  // shows the add as a touchpoint. Failures here are logged but
  // never roll back the contacts that already landed — losing a
  // log row is less bad than orphaning the records.
  ...
  if (insertedRows.length > 0) {
    ...
    if (logErr) console.warn('[contacts/bulk] data-entry log insert failed:', logErr.message);
