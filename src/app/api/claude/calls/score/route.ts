...
// POST /api/claude/calls/score
// Body: { callId: string, call: { ...CTM call fields }, force?: boolean }
// Returns a Claude-generated AI analysis of the call with: score (0–100),
// inferred caller name + interest, operator strengths + weaknesses, summary,
// and recommended next steps. Admin only. Results are cached in
// public.call_ai_scores keyed by call id.
...
function buildPrompt(call: CallInput): string {
  return `You are an expert call-center performance analyst for Seven Arrows Recovery, an addiction treatment facility. You are reviewing a single inbound or outbound phone call based on the available metadata. Produce a realistic, grounded assessment.
...
- Existing notes: ${call.notes || 'none'}
...
Respond with a single JSON object matching this schema exactly (no markdown, no prose outside the JSON):
{
  ...
  "caller_name": <best-guess name of the caller based on the metadata, or null>,
  ...
  "summary": <2-3 sentence summary of the call based on metadata: who called, likely reason, how it went>,
  "operator_strengths": [<1-4 concrete positive behaviors inferred from the call, e.g. "Kept talk time under 5 minutes — efficient intake">],
...
Rules:
- Ground the analysis in the metadata. If signals are ambiguous, say so in the summary and use "unclear" sentiment.
