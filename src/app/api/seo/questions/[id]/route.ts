...
// PATCH  /api/seo/questions/:id  → status / notes update
// DELETE /api/seo/questions/:id  → remove

export const dynamic = 'force-dynamic';

const STATUS_VALUES = ['open', 'drafting', 'live', 'ignore'] as const;
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!auth.ok) return auth.res;
  const { id } = await ctx.params;
  const { error } = await auth.supabase.from('seo_paa_questions').delete().eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 502 });
