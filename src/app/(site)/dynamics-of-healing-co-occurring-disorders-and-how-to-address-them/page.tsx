import type { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Dynamics of Healing Co Occurring Disorders and How to Address Them | Seven Arrows Recovery',
  description:
    'Did you know that the brain is one of the most complex parts of the human body? It regulates and cooperates with multiple systems, and keeps everything in balance with one another.',
  keywords:
    'dynamics of healing co occurring disorders and how to address them, dynamics of healing co occurring disorders and how to address them, addiction recovery, Seven Arrows Recovery, Arizona rehab',
  alternates: {
    canonical: 'https://sevenarrowsrecoveryarizona.com/dynamics-of-healing-co-occurring-disorders-and-how-to-address-them',
  },
};

import PageContent from './content';

export default function Page() {
  return <PageContent />;
}
