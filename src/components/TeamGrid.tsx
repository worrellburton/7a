...
import { splitCredentials } from '@/lib/displayName';
 ...
 *
 * Credentials are rendered separately from the name (they used to be
 * appended into the bold display heading with commas, so a name like
 * "Lindsay Rothschild, LCSW, CCTP, RYT-200, SAP, EMDRIA Certified"
 * ate four lines of heading-weight type). Credentials now live as
 * their own line in body font with bullet separators between each
 * one — easier to scan, and the name keeps its visual weight.
 */
export default function TeamGrid({ team }: { team: PublicTeamMember[] }) {
  if (team.length === 0) {
    return (
      <p
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4 lg:p-6">
              <h3
                className="text-white text-[16px] sm:text-lg lg:text-2xl font-bold leading-[1.1] tracking-tight relative inline-block"
              ...
              {/* Credentials — small, accent-toned, bullet-separated.
                  line-clamp-2 protects the layout when someone has six
                  credentials (we still try to keep them readable on
                  small tiles). */}
              {creds.length > 0 && (
                <p
                  className="mt-1.5 text-[10px] sm:text-[10.5px] lg:text-[11px] font-semibold text-accent/90 tracking-[0.04em] leading-[1.35] line-clamp-2"
                  style={{ fontFamily: 'var(--font-body)' }}
                >
                  {creds.map((c, i) => (
                    <span key={`${c}-${i}`} className="inline">
                      {i > 0 && (
                        <span
                          aria-hidden="true"
                          className="inline-block mx-1.5 align-middle w-1 h-1 rounded-full bg-accent/45"
                        />
                      )}
              ...
              {member.job_title && (
                <p
                  className="mt-1.5 sm:mt-2 text-white/65 text-[9px] sm:text-[10px] lg:text-[11px] font-semibold tracking-[0.18em] uppercase line-clamp-2"
                  style={{ fontFamily: 'var(--font-body)' }}
                >
                  {member.job_title}
                </p>
