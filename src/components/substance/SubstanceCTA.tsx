...
import type { SubstanceCTA as CTAContent } from '@/lib/substances/types';

/** Substance — Phase 10. Aurora CTA. */
export default function SubstanceCTA({ content }: { content: CTAContent }) {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-dark-section text-white py-28 lg:py-40">
      <div
        aria-hidden="true"
        ...
        style={{
          ...
          animation: visible ? 's-aurora 18s ease-in-out infinite alternate' : 'none',
        }}
      />
      <style>{`@keyframes s-aurora{0%{transform:translate3d(0,0,0) scale(1)}50%{transform:translate3d(-2%,1%,0) scale(1.04)}100%{transform:translate3d(2%,-1%,0) scale(1)}}`}</style>
