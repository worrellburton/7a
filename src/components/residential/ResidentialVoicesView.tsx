...
// Client-side presentation for the residential "alumni voices"
// section. The server parent (ResidentialVoices) passes real Google
// reviews in as props; this file owns the intersection-observer
// reveal animation only and never renders fabricated copy.

export interface ResidentialVoice {
  quote: string;
  author: string;
  tag: string;
}

export default function ResidentialVoicesView({ voices, averageRating }: {
  voices: ResidentialVoice[];
        ...
        >
          <div className="inline-flex items-center gap-3 bg-warm-bg rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm font-semibold text-foreground" style={{ fontFamily: 'var(--font-body)' }}>
              {averageRating.toFixed(1)} · Verified Google Reviews
            </span>
          </div>
